...
config = json.load(open("./configs/config_wv_devise.json"))
...
# model = DeviseModel(300)
model = BaseModel(300)
criterion = DeviseLossWNegative()
# train_criterion = DeviseLoss(train_data.word_vectors, train_data.target_classes, train_data.classes)
# test_criterion = DeviseLoss(test_data.word_vectors, test_data.target_classes, test_data.classes)
...
def train(epoch):
    ...
    with tqdm(train_loader, unit='batch', bar_format="{l_bar}{bar}{n_fmt}/{total_fmt} [{elapsed}] {postfix}") as tepoch:
        for batch_idx, (inputs, target_wvs, targets, negatives) in enumerate(tepoch):
            ...
            inputs, target_wvs, negatives = inputs.to(device), target_wvs.to(device), negatives.to(device)
            optimizer.zero_grad()
            outputs = model(inputs)
            # loss = train_criterion(outputs, target_wvs, targets)
            loss = criterion(outputs, target_wvs, negatives)
            loss.backward()
            optimizer.step()
    ...
    with torch.no_grad():
        with tqdm(test_loader, unit='batch') as tepoch:
            for batch_idx, (inputs, target_wvs, targets, negatives) in enumerate(tepoch):
                ...
                outputs = model(inputs)
                loss = criterion(outputs, target_wvs, negatives)
                # loss = test_criterion(outputs, target_wvs, targets)
